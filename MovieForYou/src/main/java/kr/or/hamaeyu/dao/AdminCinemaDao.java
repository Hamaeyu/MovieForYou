...
public class AdminCinemaDao {
    ...
    public List<CinemaFullInfo> getPagedTheaters(int offset, int limit) {
        List<CinemaFullInfo> list = new ArrayList<>();
        String sql = "select * from cinema c"
	    		+ "    left outer join region r on c.region_id = r.id"
	    		+ "    left outer join cinema_brand b on c.cinema_type_id = b.id"
    ...
    public int updateTheater(int id, String name, String address, String lat, String lng, int typeId, int regionId) {
        String sql =  """
				UPDATE CINEMA
				SET CINEMA_NAME = ?,CINEMA_ADDRESS = ?, LATITUDE = ?, LONGITUDE = ?,
				    CINEMA_TYPE_ID = ?,
				    REGION_ID = ?
				WHERE ID = ?
        	""";
