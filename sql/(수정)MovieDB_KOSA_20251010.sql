...
create table app_user(
    ...
    created_at  timestamp    default systimestamp not null,
    role_id    number(1)     default 2 not null --fk 일반회원이 기본값
...
comment on column app_user.role_id is '권한 번호(FK : role.id)';

--게시판 타입 테이블
comment on table post_type is '게시판 타입 마스터 테이블';
comment on column post_type.id is '게시판 타입 번호(PK)';
comment on column post_type.type_name is '타입명';

--게시판 테이블
comment on table post is '게시판 테이블 - 공통, 부모 테이블';
...
comment on column review_post.id is '게시글 번호(식별 관계 PK, FK : post.id)';
